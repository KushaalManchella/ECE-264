...
double mid_point_numerical_integration(double lower_limit, double upper_limit, int n_intervals)
{
   ...
   double interval = 0.0;
   int count = 0;
   double lower_lim = 0.0; // the value of the lower_limit which is used in calculations
   double integral_check = 0.0; // stores the value of each summation temporarily for
   ...
   while(count < n_intervals)		// mid point method of integration
   {
   		integral_check = function_to_be_integrated((lower_lim + lower_lim + interval)/2.0); 
		lower_lim += interval;
		integral += integral_check;
...
{  
   double integral = 0.0;
   double interval = 0.0;
   double integral_check = 0.0; 		// stores each summation temporarily 
   double lower_lim = 0.0; 				// stores the value of the lower_limit for calculation
   int count = 0;
  ...
  // if(n_intervals < 1)
   ...
   while(count < n_intervals) // trapezoidal formula for integration
   {
   		integral_check = ( (function_to_be_integrated(lower_lim) + function_to_be_integrated(lower_lim + interval))/2.0 );
   		lower_lim += interval;
		integral += integral_check;
		count += 1;
